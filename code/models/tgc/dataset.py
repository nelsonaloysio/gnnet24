...
class TGCDataSet(Dataset):
    def __init__(self, file_path, neg_size, hist_len, feature_path, directed, transform=None, static=False):
        ...
        with open(file_path, 'r') as infile:
            for line in infile:
                ...
                s_node = int(parts[0])
                t_node = int(parts[1])
                d_time = float(0 if static else parts[2])
    ...
    def init_neg_table(self):
        ...
        for k in range(self.node_dim):
            tot_sum += np.power(self.degrees.get(k, 1), self.NEG_SAMPLING_POWER)
        for k in range(self.neg_table_size):
            if (k + 1.) / self.neg_table_size > por:
                cur_sum += np.power(self.degrees.get(n_id, 1),self.NEG_SAMPLING_POWER)
                por = cur_sum / (tot_sum or 1)
                n_id += 1
