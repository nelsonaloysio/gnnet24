...
MODELS = sorted(os.listdir(osp.join(PATH, "models")))
...
def main(argv: list = sys.argv[1:]) -> None:
    ...
    pretrained_features = osp.join(root, args.name, "raw", args.pretrained_features or "", "x")

    data = GNNetDataset(root=root, name=args.name, transform=partial(
        transform,
        pretrained_features=pretrained_features if args.pretrained_features else None,
