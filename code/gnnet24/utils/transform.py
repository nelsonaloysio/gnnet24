...
def transform(
    ...
    :param pretrained_features: Choice of pretrained node features to load
        ...
        If `False`, use one-hot encoding (identity matrix) as node features.
    ...
    if pretrained_features is False:
        data.x = torch.eye(data.num_nodes, dtype=torch.int8)
    ...
    if split:
        ...
        if train_ratio:
            train_mask, val_mask, test_mask = temporal_edge_split(
                data, level="edge", train_ratio=train_ratio, val_ratio=val_ratio)
