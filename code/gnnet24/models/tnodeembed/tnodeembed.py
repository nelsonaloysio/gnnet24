...
from .graph_utils import get_graph_T, get_pivot_time, get_graph_times, multigraph2graph
from .task_loader import load_task
from .task_model import TaskModel, TLP, NC
from ..node2vec.cpu import Node2Vec


class tNodeEmbed(TaskModel):
    def __init__(self, graph_nx, task=NC, dump_folder=None, test_size=0, align=True, **n2vargs):
        '''
        tNodeEmbed init
        Args:
    ...
    def _initialize_embeddings(graph_nx, times=None, **n2vargs):
        ...
        for time in tqdm(times, desc='Time Steps', unit='time_step'):
            ...
            ntv_model = Node2Vec(**n2vargs)(cur_graph_nx)
            ntv = {node: ntv_model.word2vec.wv[str(node)] for node in cur_graph_nx.nodes()}
