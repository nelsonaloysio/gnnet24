...
class Node2Vec():
    ...
    :param r: Lower values promote graph exploration. Used as an
        alternative to `p`, `q`, and `c`. Default: 1.0.
    :param t: Lower values promote temporal exploration. Used only
        when `temporal` is set as `True`. Default: 0.
    ...
    def walk(self, G: nx.Graph, nodes: Optional[list] = None) -> list:
        ...
        # Ensure symmetric transition probabilities for digraphs.
