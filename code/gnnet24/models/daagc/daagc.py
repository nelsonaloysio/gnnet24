...
class DAAGC(nn.Module):
    ...
    def trainer(self, dataset: Dataset, stage: Literal["train", "pretrain"], state_dict_path: Optional[str] = None) -> dict:
        ...
        temporal_node_index_train = get_utg_node_index(dataset, mask=train)
        temporal_node_index_val = get_utg_node_index(dataset, mask=val)
        temporal_node_index_test = get_utg_node_index(dataset, mask=test)

        y = dataset.y.cpu().numpy()
        y_train = get_utg_array(y[train], temporal_node_index_train)
        y_val = get_utg_array(y[val], temporal_node_index_val)
        y_test = get_utg_array(y[test], temporal_node_index_test)
        ...
        def _pretrain():
            for epoch in range(self.max_epoch):
                ...
                # Evaluation.
                z = get_utg_tensor(z, temporal_node_index_train, how=self.agg_method)
                ...
                with torch.no_grad():
                    ...
                    z = get_utg_tensor(z[val_], temporal_node_index_val, how=self.agg_method)
                    y_pred = self.kmeans.fit_predict(z)
            ...
            with torch.no_grad():
                ...
                z = get_utg_tensor(z[test], temporal_node_index_test, how=self.agg_method)
                y_pred = self.kmeans.fit_predict(z)
        ...
        def _train():
            ...
            for epoch in range(self.max_epoch):
                ...
                Q = get_utg_tensor(Q.detach(), temporal_node_index_train, how=self.agg_method)
                ...
                with torch.no_grad():
                    ...
                    Q = get_utg_tensor(Q[val_].detach(), temporal_node_index_val, how=self.agg_method)
                    y_pred = Q.detach().data.cpu().numpy().argmax(1)
            ...
            with torch.no_grad():
                ...
                Q = get_utg_tensor(Q[test].detach(), temporal_node_index_test, how=self.agg_method)
                y_pred = Q.detach().data.cpu().numpy().argmax(1)
